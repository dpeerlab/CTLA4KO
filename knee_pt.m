function [res_x, idx_of_result] = knee_pt(y,x,make_plot, just_return)
...
issue_errors_p = true;
if (nargin > 3 && ~isempty(just_return) && just_return)
...
for breakpt = 2:length(y-1)
    ...
    if make_plot == 1
        figure;
        X = linspace(2, 50, 1000);
        plot(x, y, '.', 'MarkerSize', 20);
        hold on
        plot(X, mfwd(breakpt)*X+bfwd(breakpt), 'k-', 'LineWidth', 2);
        plot(X, mbck(breakpt)*X+bbck(breakpt), 'r-', 'LineWidth', 2);
        xlim([min(x), max(x)])
        ylim([min(y), max(y)])
    end
end
