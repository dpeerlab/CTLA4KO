...
% -----------------------------------------------------------------------
function [labels,G,c,Q] = par_phenograph( data, k, varargin )
...
if ~exist('IDX','var') && ~exist('G','var')
    ...
    %IDX(:,1) = []; D(:,1) = [];
    lnn = parfor_spdists_knngraph( data, k, 'distance', distance, 'chunk_size', min(chunk_size, size( data, 1 )), 'SNN', true, 'verbose', true );
    [IDX_row, ~] = find(lnn > 0);
    IDX = reshape(IDX_row, k, size( data, 1 ));
    IDX = IDX';
